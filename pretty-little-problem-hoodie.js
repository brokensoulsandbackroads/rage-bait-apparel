(() => {
  const PRODUCT_NAME = 'Pretty Little Problem Hoodie';
  const SIZES = ['XS', 'S', 'M', 'L', 'XL', '2XL'];
  const SIZE_SKUS = Object.fromEntries(SIZES.map(size => [size, `JH001F-HPK-${size}`]));
  const PRICE = 39.99;
  const CART_KEY = 'ragebait-cart-v1';

  const productGrid = document.querySelector('.product-grid');
  if (!productGrid || productGrid.querySelector('[data-product-card="pretty-little-problem-hoodie"]')) return;
  ...
  card.innerHTML = `
    ...
    <div class="product-info">
      <div>
        <h3>${PRODUCT_NAME}</h3>
        <p>Women's hot pink Pretty Little Problem hoodie with front + back graphics<span style="display:block;margin-top:5px;color:#7f887b;font-size:10px;letter-spacing:.03em;">VAT included · shipping extra · women's fit</span></p>
      </div>
    ...
    <label class="hoodie-size-picker" style="display:flex;align-items:center;gap:10px;padding:12px 14px;border-top:1px solid var(--line);background:#0f110f;color:#b9c0b4;font-size:11px;font-weight:900;text-transform:uppercase;letter-spacing:.08em;">
      <span>Size</span>
      <select aria-label="Choose Pretty Little Problem Hoodie size" style="flex:1;min-width:0;background:#080908;color:#fff;border:1px solid #394038;padding:9px 10px;font-weight:800;">
  ...
  const outlawHoodie = productGrid.querySelector('[data-product-card="outlaw-hoodie"]');
  if (outlawHoodie?.nextSibling) productGrid.insertBefore(card, outlawHoodie.nextSibling);
  else if (outlawHoodie) productGrid.appendChild(card);
  ...
  Promise.all(galleryImages.map(preload)).then(results => {
    ...
    const prev = arrow('Previous Pretty Little Problem Hoodie photo', '‹', 'left');
    const next = arrow('Next Pretty Little Problem Hoodie photo', '›', 'right');
    ...
    const dotEls = loaded.map((_, index) => {
      ...
      dot.type = 'button';
      dot.setAttribute('aria-label', `Show Pretty Little Problem Hoodie photo ${index + 1} of ${loaded.length}`);
